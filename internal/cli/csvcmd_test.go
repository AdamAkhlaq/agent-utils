...
// json2csv shares the sep-flag CLI shape with csv2json but must not append a
// newline: the CSV writer already terminates every record.
func TestJSONToCSVCommand(t *testing.T) {
	tests := []struct {
		name      string
		args      []string
		stdin     string
		...
		{name: "default comma", args: nil, stdin: "X", want: "X", wantSep: ','},
		{name: "semicolon separator", args: []string{"-sep", ";"}, stdin: "X", want: "X", wantSep: ';'},
		{name: "backslash t means tab", args: []string{"-sep", `\t`}, stdin: "X", want: "X", wantSep: '\t'},
		{name: "multi-character separator", args: []string{"-sep", "ab"}, wantErr: true, wantUsage: true},
		{name: "empty separator", args: []string{"-sep", ""}, wantErr: true, wantUsage: true},
		{name: "bad flag", args: []string{"-x"}, wantErr: true, wantUsage: true},
		{name: "two file arguments", args: []string{"a", "b"}, wantErr: true, wantUsage: true},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			var gotSep rune
			cmd := JSONToCSVCommand(func(w io.Writer, r io.Reader, sep rune) error {
				gotSep = sep
				_, err := io.Copy(w, r)
				return err
			})
...
func TestJSONToCSVCommandConversionFailureIsRuntimeError(t *testing.T) {
	broken := errors.New("input must be a JSON array of objects, got a string")
	cmd := JSONToCSVCommand(func(w io.Writer, r io.Reader, sep rune) error { return broken })
	var stdout, stderr bytes.Buffer
